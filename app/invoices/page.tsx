...
export default function InvoicesPage() {
  ...
  const fetchMasterData = async () => {
    try {
      console.log('Fetching master data...'); // Debug log
      ...
      console.log('Customers response:', customersRes.data); // Debug log
      console.log('HSN codes response:', hsnRes.data); // Debug log
  ...
  const updateInvoiceItem = (index: number, field: string, value: any) => {
    ...
    // Auto-populate GST rate when HSN code is selected
    if (field === 'hsn_code') {
      const selectedHsn = hsnCodes.find(hsn => hsn.hsn_sac_code === value);
      if (selectedHsn) {
        updatedItems[index].gst_rate = parseFloat(selectedHsn.gst_rate);
      }
    }
  ...
  const handleCreateInvoice = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const invoiceData = {
      ...
      submit_to_gst: formData.get('submit_to_gst') === 'true',
      items: invoiceItems
    };
                  ...
                  <form onSubmit={handleCreateInvoice} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        {customers.length === 0 && (
                          <p className="text-sm text-red-500">No customers loaded. Check network connection.</p>
                        )}
                      </div>
                      ...
                      <div className="space-y-2">
                        <label className="block text-sm font-medium text-gray-700">
                          Place of Supply (State Code) <span className="text-red-500 ml-1">*</span>
                        </label>
                        <input
                          type="text"
                          name="place_of_supply"
                      ...
                      <div className="space-y-2">
                        ...
                        >
                          ...
                          <option value="true">Yes - Submit to GST Portal</option>
                        </select>
                      </div>
                    </div>
                          ...
                          <div key={index} className="border border-gray-200 p-4 rounded-lg">
                            <div className="grid grid-cols-1 md:grid-cols-6 gap-4 mb-2">
                              ...
                              <div>
                                ...
                                {hsnCodes.length === 0 && (
                                  <p className="text-xs text-red-500 mt-1">No HSN codes loaded</p>
                                )}
                              </div>
